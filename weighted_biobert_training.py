...
class bertWithCustomLoss(nn.Module):
    def __init__(self, model_dir, num_labels, id2label, label2id):
    ...
    def forward(self, input_ids, attention_mask, labels, classifiers, clf_count, device, weight_func=None):
        outputs = self.biobert(input_ids, attention_mask=attention_mask, output_hidden_states=True)
        logits = outputs[0]
        weights = torch.zeros(labels.shape).to(device)
        committee_logits = outputs.hidden_states
        committee_logits = committee_logits[-1]
        for i in range(clf_count):
                curr_clf = classifiers[i]
                output = curr_clf(committee_logits)
                out_soft = torch.nn.Softmax(dim=2)(output)
                out_pos = torch.argmax(out_soft, dim=2)
                tmp = (out_pos == labels).float()
                tmp = torch.tensor(tmp)
                weights += tmp

        if weight_func == 'linear_reweight':
            clf_cutoff_cnt = float(clf_count - 2)
            weights /= clf_count
            weights[weights > (clf_cutoff_cnt/clf_count)] = 0
            weights = 1 - weights
        elif weight_func == 'non_linear_reweight': 
            # non-linear reweight
            alpha = 1
            weights[weights >= clf_count-2] = 0
            weights /= clf_count
            weights = 1 / (weights + alpha)

        loss = torch.nn.CrossEntropyLoss(reduction='none')(logits.view(-1, self.num_labels), labels.view(-1))
...
def weighted_loss_train(model, dataloader, classifiers, optimizers, bert_optim, device, clf_count, weight_func):
    tr_loss = 0
    ...
    for idx, batch in enumerate(dataloader):
        ...
        outputs = model(input_ids, mask, targets, classifiers, clf_count, device, weight_func)
        ...
        if idx % 50 == 0:
            print(f'\tLoss after {idx} steps is {tr_loss}')
...
def inference(model, dataloader, tokenizer, device, id2label):
    ...
    for batch in dataloader:
        ...
        output_bert = model(ids, mask, targets, [], 0, device)
...
def get_validation_loss(model, dataloader, device):
    model.eval()
    val_loss = 0
    ...
    for batch in dataloader:
        ...
        # self, input_ids, attention_mask, labels, classifiers, clf_count, device, weight_func=None)
        output = model(input_ids, mask, targets, [], 0, device)
...
def main():
    ...
    model = bertWithCustomLoss(model_dir, num_labels, id2label, label2id)
    ...
    for i in range(args.ensemble_size):
        ...
        curr_classifier = create_classifier(768, num_labels)
    ...
    optimizers = []
    bert_optim = torch.optim.Adam(params=model.parameters(), lr=LEARNING_RATE)
    min_val_loss = float('inf')
    for i in range(args.num_epochs):
        print(f'Epoch {i+1}:')
        weighted_loss_train(model, train_dataloader, classifiers, optimizers, bert_optim, device, args.ensemble_size, args.weight_func)
        ...
        print(f'\tGenerated prediction file')
        val_loss = get_validation_loss(model, devel_dataloader, device)
        print(f'Validation loss for epoch {i} : {val_loss}')
        if val_loss < min_val_loss:
            min_val_loss = val_loss
            # m_name = 'model_ep_' + str(i) + '_' + args.weight_func
            m_name = 'model_min_valLoss_' + args.weight_func
            m_path = os.path.join('resources', args.dataset_name, m_name)
            print(f'Model with minimum validation loss, saved at epoch {i}')
        ...
        with open('tmp.txt', 'a') as fh:
            ...
            fh.write(f'Val loss: {val_loss}\n')

    m_name = 'model_' + args.weight_func + '_' + str(args.num_epochs)
    m_path = os.path.join('resources', args.dataset_name, m_name)
    model.save_model(m_path)
